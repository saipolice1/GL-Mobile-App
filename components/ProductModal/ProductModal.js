...
export const ProductModal = ({ 
  ...
  useEffect(() => {
    ...
    if (visible && product) {
      ...
      isFavorite(product._id).then(setIsProductFavorite);
      
      addToRecentlyViewed(product).then(() => {
        // Fetch updated recently viewed list
        getRecentlyViewed().then(items => {
          // Filter out current product
          setRecentlyViewed(items.filter(p => p._id !== product._id).slice(0, 6));
        });
      });
    }
  }, [product?._id, visible]);
  ...
  useEffect(() => {
    const unsubscribe = subscribeFavorites(async () => {
      if (product?._id) {
        const isFav = await isFavorite(product._id);
  ...
  const productCollectionId = product?.collectionIds?.[0] || collectionId;

  // Fetch similar products based on the product's own category
  const { data: similarProducts = [] } = useQuery({
    queryKey: ['similarProducts', productCollectionId, product?._id],
    queryFn: async () => {
      try {
        if (!productCollectionId) {
          ...
          return (response?.items || []).filter(p => p._id !== product?._id).slice(0, 6);
        }
  ...
  const getPrice = () => {
    const priceAmount = Number.parseFloat(product?.priceData?.price) || 0;
  ...
  const unitPrice = Number.parseFloat(product?.priceData?.price) || 0;
  
  // Get stock information
  const stockQuantity = product?.stock?.quantity;
  const inStock = product?.stock?.inStock !== false;
  const trackInventory = product?.stock?.trackQuantity !== false;
            ...
            <View style={styles.productHeader}>
              ...
              <View style={styles.productInfo}>
                ...
                <Text style={styles.productPrice}>
                  {product.priceData?.formatted?.price || formatPrice(unitPrice)}
                </Text>

...
export const ShopCollectionsHome = ({ navigation }) => {
  ...
  const collectionsResponse = useQuery({
    queryKey: ["collections"],
    queryFn: () => wixCient.collections.queryCollections().find(),
  });
  ...
  return (
    <View style={[styles.view, localStyles.view]}>
      <Text style={[styles.title, localStyles.title]}>Shop by Category</Text>
      ...
      <View style={[styles.carouselContainer, localStyles.carouselContainer]}>
        <Text
          style={[
            styles.title,
            localStyles.title,
        ...
        <Carousel
          ...
          renderItem={({ index }) => {
            return (
              <View style={[styles.carousel, localStyles.carousel]}>
                <Pressable
                  onPress={() => {
                    navigation.navigate(Routes.Store, {
                ...
                >
                  <Image
                    style={[styles.carouselImage, localStyles.carouselImage]}
                    source={{
                      uri:
                  ...
                  />
                  <Text style={[styles.carouselTitle, localStyles.carouselTitle]}>
                    {filteredProducts[index].name}
                  </Text>
                  <Text style={[styles.carouselPrice, localStyles.carouselPrice]}>
                    {
        ...
        <View style={styles.indicators}>
          ...
          >
            <IconButton icon={"chevron-left"} size={30} iconColor={theme.colors.gold} />
          </TouchableOpacity>
          <TouchableOpacity
            onPress={handleNext}
            style={styles.indicatorButtonRight}
          >
            <IconButton icon={"chevron-right"} size={30} iconColor={theme.colors.gold} />
          </TouchableOpacity>
        </View>
      </View>
    </View>
